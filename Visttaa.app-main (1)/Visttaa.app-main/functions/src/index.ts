...
export const updateSeller = onCall(async request => {
  ...
  const changes = { nome, email, role: 'seller', updatedAt: new Date().toISOString(), updatedBy: uid };
...
function validateItems(items: unknown): SaleItem[] {
  ...
  const validItems = items.map((item: SaleItem) => ({ id: String(item.id || ''), qtd: numberOrError(item.qtd, 'Quantidade', 0.000001), venda: 0, custo: 0, codigo: '', marca: '', modelo: '' })).filter(item => item.id);
