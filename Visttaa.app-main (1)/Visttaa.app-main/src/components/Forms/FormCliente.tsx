...
export function FormCliente({ data, onSave, onClose, onDirtyChange }: FormClienteProps) {
  ...
  const clienteVazio = () => ({ nome: '', cpf: '', tel: '', nasc: '', email: '', whatsappConsent: false, ultimaCompraOculos: '', ultimaCompraLente: '', lembreteOculosEm: '', lembreteLenteEm: '', endereco: { cep: '', logradouro: '', numero: '', complemento: '', bairro: '', cidade: '', estado: '' }, prescricao: receitaVazia() });
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div><label className={labelClass}>Última compra de óculos</label><input type="date" value={form.ultimaCompraOculos} onChange={e=>handleChange('ultimaCompraOculos', e.target.value)} className={inputClass} /></div>
            <div><label className={labelClass}>Lembrete de óculos</label><input type="date" value={form.lembreteOculosEm} onChange={e=>handleChange('lembreteOculosEm', e.target.value)} className={inputClass} /></div>
            <div><label className={labelClass}>Última compra de lentes</label><input type="date" value={form.ultimaCompraLente} onChange={e=>handleChange('ultimaCompraLente', e.target.value)} className={inputClass} /></div>
            <div><label className={labelClass}>Lembrete de lentes</label><input type="date" value={form.lembreteLenteEm} onChange={e=>handleChange('lembreteLenteEm', e.target.value)} className={inputClass} /></div>
            <label className="md:col-span-2 flex items-center gap-3 text-sm font-semibold text-slate-600"><input type="checkbox" checked={Boolean(form.whatsappConsent)} onChange={e=>handleChange('whatsappConsent', e.target.checked)} className="h-4 w-4" /> Cliente autorizou contato sobre revisão e reposição pelo WhatsApp</label>
          </div>
        </div>
