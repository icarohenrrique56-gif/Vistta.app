...
export function CadastrosGenericosScreen({ activeTab }: { activeTab: string }) {
  const { fornecedores, contas, categorias, usuarios, salvarCadastro, excluirCadastro, alterarStatusVendedor, redefinirAcessoVendedor } = useAppContext();
  ...
  const configs: Record<string, any> = {
    fornecedores: { defaultData: { razaoSocial: '', nomeFantasia: '', cnpj: '', inscricaoEstadual: '', telefone: '', email: '', contatoComercial: '', categoriaFornecimento: 'Armações', prazoEntrega: '', condicoesComerciais: '', parceriaAtiva: true, enderecoCep: '', enderecoLogradouro: '', enderecoNumero: '', enderecoComplemento: '', enderecoBairro: '', enderecoCidade: '', enderecoEstado: '' }, fields: [
      ...
      { name: 'enderecoCep', label: 'CEP', type: 'text', mask: 'cep' },
    ...
    usuarios: { submitLabel: 'Criar vendedor', busyLabel: 'Salvando vendedor...', defaultData: { nome: '', email: '', senha: '', confirmarSenha: '' }, fields: [
      { name: 'nome', label: 'Nome', type: 'text', required: true },
  ...
  const titles: Record<string, string> = { fornecedores: 'Fornecedores', contas: 'Contas', categorias: 'Categorias', usuarios: 'Vendedores' };
        ...
        <div className="overflow-x-auto overflow-y-auto flex-1 custom-scrollbar">
            ...
            <tbody className="divide-y divide-slate-50">
                ...
                <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    {item.valor ? <span className={`font-extrabold ${item.tipo==='pagar'?'text-rose-500':'text-emerald-500'}`}>{formatMoney(item.valor)}</span> : activeTab === 'usuarios' ? <span className={item.status === 'active' ? 'text-emerald-600' : 'text-rose-500'}>{item.status === 'active' ? 'Ativo' : 'Inativo'} · Vendedor</span> : (item.contato || item.telefone || item.categoriaFornecimento || item.perfil || item.descricao || '-')}{activeTab === 'fornecedores' && <span className="block text-[11px] text-slate-400 mt-1">{contas.filter((conta: any) => conta.fornecedorId === item.id).length} compra(s) vinculada(s)</span>}
                    </td>
                    ...
                    <div className="flex justify-center gap-2">
                        ...
                        {activeTab === 'usuarios' ? <>
                          <button title={item.status === 'active' ? 'Desativar vendedor' : 'Ativar vendedor'} onClick={() => alterarStatusVendedor(item.authUid || item.id, item.status === 'active' ? 'inactive' : 'active').catch((error: any) => alert(error.message))} className="rounded-xl px-2 text-xs font-bold text-slate-500 hover:bg-slate-100">{item.status === 'active' ? 'Desativar' : 'Ativar'}</button>
                          <button title="Enviar redefinição de acesso" onClick={() => redefinirAcessoVendedor(item.email).then(() => alert('E-mail de redefinição enviado.')).catch((error: any) => alert(error.message))} className="rounded-xl px-2 text-xs font-bold text-slate-500 hover:bg-slate-100">Redefinir</button>
