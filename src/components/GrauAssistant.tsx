...
import { ArrowRight, Bot, CircleHelp, Send, X } from 'lucide-react';
...
function answerQuestion(question: string, tab: string, role: string | null): GrauMessage {
...
export function GrauAssistant() {
  const { activeTab, userRole, setActiveTab } = useAppContext();
  ...
  const submit = (event: React.FormEvent) => {
    ...
    if (!trimmed) return;
    setMessages(previous => [...previous, { from: 'user', text: trimmed }, answerQuestion(trimmed, activeTab, userRole)]);
    setQuestion('');
  };

  const ask = (text: string) => {
    setMessages(previous => [...previous, { from: 'user', text }, answerQuestion(text, activeTab, userRole)]);
  };

  return <>
    <button type="button" onClick={openAssistant} title="Abrir GRAU" aria-label="Abrir GRAU" className="fixed bottom-5 right-5 z-[60] flex items-center gap-2 rounded-full bg-[var(--vistta-plum)] px-4 py-3 text-sm font-bold text-white shadow-[0_12px_30px_rgba(48,32,77,.25)] transition-transform hover:-translate-y-0.5 hover:bg-[var(--vistta-violet)]">
      <Bot size={18} /> GRAU 🤖
    </button>
    {open && <div className="fixed inset-0 z-[80] bg-slate-950/30" onMouseDown={event => { if (event.target === event.currentTarget) setOpen(false); }}>
      <section className="absolute bottom-4 right-4 flex max-h-[min(680px,calc(100dvh-2rem))] w-[min(390px,calc(100vw-2rem))] flex-col overflow-hidden rounded-[26px] border border-[var(--vistta-border)] bg-[var(--vistta-surface)] shadow-[0_24px_70px_rgba(15,11,36,.3)]" role="dialog" aria-modal="true" aria-label="GRAU">
        <header className="flex items-center justify-between border-b border-[var(--vistta-border)] bg-[var(--vistta-plum)] px-5 py-4 text-white">
          ...
          <button type="button" onClick={() => setOpen(false)} aria-label="Fechar GRAU" className="rounded-full p-2 text-white/75 hover:bg-white/10 hover:text-white"><X size={18} /></button>
        </header>
