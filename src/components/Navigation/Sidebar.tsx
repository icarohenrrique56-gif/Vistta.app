  ...
  return (
    <aside className={`hidden md:flex flex-col ${collapsed ? 'w-[88px]' : 'w-[270px]'} bg-[#30204d] text-white z-20 transition-all duration-300 shadow-[12px_0_35px_rgba(48,32,77,.08)]`}>
      ...
      <div className={`flex-1 overflow-y-auto py-6 ${collapsed ? 'px-3' : 'px-4'} space-y-1 custom-scrollbar`}>
        ...
        {userRole !== 'seller' && <SidebarItem icon={Wallet} label="Caixa Diário" active={activeTab === 'caixa'} onClick={() => setActiveTab('caixa')} badge={caixaAberto ? 'Aberto' : 'Fechado'} badgeColor={caixaAberto ? 'bg-[#c6ed76] text-[#30204d]' : 'bg-white/10 text-white/50'} collapsed={collapsed} />}
        <SidebarItem icon={ShoppingCart} label="PDV" active={activeTab === 'vendas'} onClick={() => setActiveTab('vendas')} collapsed={collapsed} />
        {userRole === 'seller' && <SidebarItem icon={FileText} label="Minhas Vendas" active={activeTab === 'minhas-vendas'} onClick={() => setActiveTab('minhas-vendas')} collapsed={collapsed} />}
        {userRole !== 'seller' && <SidebarItem icon={FileText} label="Orçamentos" active={activeTab === 'orcamentos'} onClick={() => setActiveTab('orcamentos')} badge={orcamentos.filter((o: Orcamento) => o.status === 'pendente').length || null} badgeColor="bg-[#f4c96b] text-[#30204d]" collapsed={collapsed} />}
        {userRole !== 'seller' && <SidebarItem icon={FileText} label="Ordens de Serviço" active={activeTab === 'ordens'} onClick={() => setActiveTab('ordens')} collapsed={collapsed} />}
        ...
        {userRole !== 'seller' && <SidebarItem icon={Tags} label="Categorias" active={activeTab === 'categorias'} onClick={() => setActiveTab('categorias')} collapsed={collapsed} />}
      ...
      <div className={`p-4 border-t border-white/10 flex items-center ${collapsed ? 'justify-center' : 'justify-between'}`}>
        <div className="flex items-center min-w-0">
          ...
          {!collapsed && <div className="min-w-0"><div className="text-sm font-bold truncate text-white">{user?.email?.split('@')[0] || 'Usuário'}</div><div className="text-[10px] text-white/45 font-bold uppercase">{userRole === 'admin' ? 'Administrador' : userRole === 'manager' ? 'Gestor' : 'Vendedor'}</div></div>}
        </div>
