...
Use esta pasta como a raiz única do projeto:

```text
Visttaa.app-main (1)/Visttaa.app-main
```

Existe uma cópia antiga em `Vistta.app-main (2)/Vistta.app-main`; ela não contém as correções e integrações atuais e não deve ser usada para executar ou publicar o sistema.
...
O projeto Firebase selecionado para deploy está definido em `.firebaserc` como `vistta-2e1df`. Para conectar a aplicação web, copie `.env.example` para `.env` e preencha os valores da configuração Web exibida em **Firebase Console > Configurações do projeto > Seus apps**:

```bash
cp .env.example .env
```

Para deploy, autentique a CLI primeiro. Sem essa autenticação não é possível consultar o projeto nem publicar regras, funções ou hosting:

```bash
functions/node_modules/.bin/firebase login
functions/node_modules/.bin/firebase use vistta-2e1df
```
